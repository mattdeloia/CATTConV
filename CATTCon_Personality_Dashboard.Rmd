...
```{r message=FALSE, warning=FALSE, echo=FALSE}
df2a2 %>% filter(Dimension != "SelfEfficacy", CyberPro %in% c("Yes", "Control")) %>% 
  ggplot() +
  geom_density(aes(x=Score, fill=CyberPro)) +
  facet_wrap(Dimension~., ncol=4) + geom_vline(aes(xintercept=.50)) +
  xlim (0,1) +
  xlab ("Raw Score Normalized") +
  theme(legend.title= element_text(color="white", size=0), legend.position = "blank") +
  scale_fill_manual(values=c("red", "lightgreen", "darkgray"))
...
df_sd <- dfcluster3a %>% group_by(Category) %>% summarise(sd=sd(Score), mean=mean(Score)) %>% mutate(cluster_sd=if_else(sd>.075, "High", "Low"))

dfcluster3a <- dfcluster3a %>% left_join(df_sd) %>% mutate(score=if_else((mean-Score)>.05, "Deviation", "Normal"))
...
##### *Cluster 1* (`r 100*round(28/102,2)`%) and *Cluster 3* (`r 100*round(30/102,2)`%) , consists of respondents with lower scores on many dimensions.  Cluster 1 may best resemble "Avoiders" and and Cluster 3 "Empathizers".
