...
```{r, echo=FALSE}
df_ed2 %>% ggplot() + geom_col(aes(x=Education, y=n, fill= Education)) + 
  ylab("Percentage of Particpants") +
  geom_text(aes( x=Education, y=n, label = round(n, 2), ),  hjust = .5, vjust = -.1 , size=3.5, color="red") +
  xlab ("") +
  scale_fill_manual(name="Field", values = c("lightgray", "gray", "lightgreen", "green"))+
...
corrplot(cor(dfcluster), method="color", order="hclust", type="full", addrect=4, cl.lim=c(-1,1), addCoef.col="black", rect.col="green", diag = FALSE, number.font=1, number.digits = 1, number.cex = .7)
```
